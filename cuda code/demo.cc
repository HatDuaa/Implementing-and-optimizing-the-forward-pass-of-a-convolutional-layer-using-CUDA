...
void test()
{
    ...
    // host
    std::cout << "Test using Lenet-5 on host:" << std::endl;
    Network Lenet5_host = Network_lenet5_host();
    Lenet5_host.load_parameters("./build/save-weights.bin");

    Lenet5_host.forward(dataset.test_data);
    float acc_host = compute_accuracy(Lenet5_host.output(), dataset.test_labels);
    std::cout << "test acc: " << acc_host << std::endl;
    std::cout << "==============================" << std::endl;
